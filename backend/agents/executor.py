    ...
    async def process_alert(self, alert_id: int, symbol: str, current_price: float, drawdown: float):
        ...
        try:
            ...
            if not get_setting("autonomous_hedging", True):
                logger.warning(f"⏸️ Autonomous hedging disabled — alert #{alert_id} acknowledged, no order placed")
                log_event("Executor", "HEDGING_DISABLED", f"Autonomous hedging disabled — alert #{alert_id} for {symbol} acknowledged, no order placed.", severity="warning")
            ...
            if premium > max_prem:
                ...
                logger.error(f"❌ Order failed: premium ${premium:.2f} exceeds max allocation ${max_prem:.2f}")
                log_event("Executor", "ORDER_FAILED", f"Premium ${premium:.2f} exceeds max allocation ${max_prem:.2f} — order rejected.", severity="error")
                return
            ...
            if order_result["success"]:
                ...
                log_event("Executor", "HEDGE_PLACED", f"Protective put order filled: {symbol} {otm:.0%} OTM, {expiry_days}-day expiry, ${premium:.2f} premium.")
            else:
                self._update_alert_status(db, alert_id, "failed")
